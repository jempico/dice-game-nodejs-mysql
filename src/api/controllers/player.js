...
class playerController { 
    ...
    async readPlayer(req, res) {
        try{
            const {id} = req.params;
            const foundPlayer = await player.getPlayer(id)
            if (foundPlayer == 0){
    ...
    async editPlayer(req, res){
        try{
            ...
            if (result == 1) {  
                res.status(200).json({  success: true, text: `user name successfully modified!` })
            } else if (result ==0) {
    ...
    async readPlayers(req, res) {
        try{
            const playersList = await player.getPlayers()
